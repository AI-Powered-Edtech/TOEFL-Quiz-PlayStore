...
use crate::services::{account, admin, admin_monitoring, ai, auth, oauth, blog, creator, monitoring, profile, quiz, social, storage, writing, purchases, oracle};
...
async fn main() {
    ...
    let auth_svc = ServiceProcess::new("auth")
        ...
        .endpoint(Method::DELETE, "/account", delete(account::delete_account))
        .state(state.clone());
