...
export const IELTSWritingSim: React.FC<{ onNavigate: (view: AppView) => void; onBack: () => void; }> = ({ onNavigate, onBack }) => {
    ...
    const [showWordCountConfirm, setShowWordCountConfirm] = useState(false);
    ...
    return (
        <div className={`h-[100dvh] flex flex-col bg-[#EEF2FC] dark:bg-slate-950 overflow-hidden ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
            ...
            {/* Word Count Confirmation Dialog */}
            {showWordCountConfirm && (
                <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm">
                    <div className="w-full max-w-lg bg-white rounded-t-2xl p-6 shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
                        <h3 className="text-base font-bold text-slate-800 mb-2">Essay terlalu pendek</h3>
                        <p className="text-sm text-slate-600 mb-6">
                            Essay kamu baru <strong>{wordCount} kata</strong>. Minimal{' '}
                            <strong>{taskType === 'Task 1' ? 150 : 250} kata</strong> untuk skor yang akurat.
                            Tetap submit?
                        </p>
                        <div className="flex gap-3">
                            <button
                                onClick={() => setShowWordCountConfirm(false)}
