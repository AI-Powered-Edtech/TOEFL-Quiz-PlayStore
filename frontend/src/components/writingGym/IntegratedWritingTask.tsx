...
export const IntegratedWritingTask: React.FC<{ onNavigate: (view: AppView) => void }> = ({ onNavigate }) => {
    ...
    const streamPlayerRef = useRef<StreamingKittenPlayer | null>(null);
    ...
    useEffect(() => {
        if (!Capacitor.isNativePlatform()) {
            warmupKittenTts().catch(() => console.warn('Silent TTS warmup fail'));
        }
    }, []);
        ...
        const saveTimer = setTimeout(() => {
            try {
                updatePersistedSession({
                    ...
                    userNotes,
                    essay
                }).catch((e: any) => { console.error('Failed to auto-save draft:', e); });
    ...
    useEffect(() => {
        ...
        // Pause timer while generating audio to prevent unfair time loss
        if (phase === 'listening' && isGeneratingAudio) return;
    ...
    }, [phase, timeRemaining, isGeneratingAudio]);
    ...
    useEffect(() => {
        if (phase === 'listening' && task && !isPlaying && !hasPlayedAudio && !audioError) {
    ...
    const generateLectureAudio = async () => {
        ...
        setAudioError(false);
        try {
            // Use Sherpa-ONNX Piper on native, Kitten TTS on web
            const callbacks = {
                ...
                onError: (err: any) => {
                    console.error('[Integrated Writing TTS] Stream error:', err);
                    setAudioError(true);
                    setShowTranscript(true);
                    setIsGeneratingAudio(false);
                    setIsPlaying(false);
            ...
            const player = Capacitor.isNativePlatform()
                ? new MobileMockPlayer(callbacks)
                : new StreamingKittenPlayer(callbacks);

            streamPlayerRef.current = player as any;
            setActiveSubtitle(Capacitor.isNativePlatform() ? 'Speaking with Piper...' : 'Booting local AI engine...');

            // Start playing
            await player.play(task.lecture.transcript, ...(Capacitor.isNativePlatform() ? [] : [1.0, 0]) as any);

            // Mark loaded if it returns fast (cache hit)
            setIsGeneratingAudio(false);
    ...
    const togglePlay = () => {
        ...
        if (isPlaying) {
            streamPlayerRef.current.stop(); // Gapless streaming doesn't support 'pause' cleanly, so we stop 
            setIsPlaying(false);
        } else {
            // Replay from start if they stopped it
            generateLectureAudio();
        }
    };
                                ...
                                {!isGeneratingAudio && !audioError && (
                                    <>
                                        ...
                                        <div className="text-center mb-6">
                                            ...
                                            <div className="h-6 flex flex-col items-center justify-center mt-2 mb-2 px-4">
                                                {isPlaying ? (
                                                    ...
                                                    <p className="text-slate-500 dark:text-slate-400 text-sm">
                                                        {hasPlayedAudio ? 'Audio Complete' : 'Click play to start'}
                                                    </p>
                                                )}


                                                {/* Progress indicator */}
                                                {isPlaying && audioProgress.total > 0 && (
                                                    <div className="w-full max-w-xs mt-2 bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden">
                                                        <div
                                                            className="h-full bg-indigo-500 transition-all duration-300"
                                                            style={{ width: `${Math.min(100, (audioProgress.current / audioProgress.total) * 100)}%` }}
                                                        />
                                                    </div>
                                                )}
                                            </div>
