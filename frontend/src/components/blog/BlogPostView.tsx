...
export const BlogPostView: React.FC<BlogPostViewProps> = ({ onNavigate, onBack, onStartSkill, postId }) => {
    ...
    const renderContent = (content: string) => {
        return content.split('\n').map((line, index) => {
            ...
            if (line.startsWith('[AUDIO:')) {
                const url = line.replace('[AUDIO:', '').replace(']', '').trim();
                return (
                    <div key={index} className="my-6 w-full max-w-md mx-auto">
                        <audio controls className="w-full rounded-full shadow-sm bg-slate-50" src={url} preload="metadata">
    ...
    useEffect(() => {
        ...
        fetchBlogPost(postId || '')
            ...
            .catch((e: any) => { console.error('Failed to fetch blog post:', e); })
            .finally(() => { if (!cancelled) setLoadingPost(false); });
        // Track views (non-critical)
        if (postId) incrementBlogPostViews(postId).catch((e: any) => { console.error('Failed to increment views:', e); });
