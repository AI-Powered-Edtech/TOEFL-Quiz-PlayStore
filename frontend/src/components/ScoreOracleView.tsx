...
export const ScoreOracleView: React.FC<ScoreOracleViewProps> = ({ onNavigate, userId }) => {
    ...
    const loadData = useCallback(async (retryCount = 0) => {
        try {
            ...
            const hist = await oracleService.getPredictionHistory(userId, 10);
            setHistory(hist);
            ...
            try {
                // Try to load CEFR results from local storage
                const cefrKey = `cefr_results_${userId}`;
                const storedCefr = localStorage.getItem(cefrKey);
                if (storedCefr) {
                    const cefrResults = JSON.parse(storedCefr);
                    setCefrResult(Array.isArray(cefrResults) ? cefrResults[0] : cefrResults);
                }
    ...
    useEffect(() => {
        if (prediction) {
            oracleService.getPredictionHistory(userId, 10).then(setHistory);
        }
