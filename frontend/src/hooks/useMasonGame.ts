...
export const useMasonGame = ({ initialLevel, onNavigate, decrementHeart, isOutOfHearts, onPaywall, onWrongAnswer }: UseMasonGameProps) => {
    // ==================== AUTH & USER ====================
    const { user } = useAuth();
    ...
    const loadNewExercise = useCallback(async (overrides?: { levelNum?: number, skillId?: string, difficulty?: 'normal' | 'hard' }) => {
        ...
        try {
            ...
            const data = await Promise.race([
                writingGymService.generateExercise('mason', effectiveSkillId as any, effectiveDifficulty),
                timeoutPromise
            ]);

            const duration = Date.now() - startTime;
            metricsService.recordTiming('MasonLevel', duration, 'exercise_load', { level: Number(effectiveLevel) });
            setExercise(data);
            ...
            console.error(error);
            loggingService.error('MasonLevel', 'Exercise load failed', { level: effectiveLevel, skillId: effectiveSkillId });
            setGameState(prev => ({ ...prev, status: 'error' }));
        } finally {
    ...
    const handleSuccess = async () => {
        metricsService.record({ metricName: 'MasonLevel', component: 'session_completed', metricValue: 1, tags: { level: String(currentLevel), finalScore: gameState.score } });
    ...
    const handleError = useCallback(() => {
        metricsService.record({ metricName: 'MasonLevel', component: 'wrong_answer', metricValue: 1, tags: { level: String(currentLevel), livesLeft: gameState.lives - 1 } });
        sound.play('error');
        masonHaptics.error();
        ...
        if (gameState.lives > 1) {
            ...
            onWrongAnswer?.(); // Trigger shake animation in UI
        } else {
            // Game Over
