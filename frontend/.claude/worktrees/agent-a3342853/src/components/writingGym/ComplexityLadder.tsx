    ...
    const submitAnswer = async () => {
        ...
        try {
            ...
            if (result.isValid) {
                ...
                // Fire-and-forget analytics — level completed
                writingGymService.logAnalyticsEvent(user?.id, 'complexity_ladder_level_completed', {
                ...
                setTimeout(() => {
                    if (currentIdx < levels.length - 1) {
                        ...
                        // All levels complete — track skill mastery
                        writingGymService.logAnalyticsEvent(user?.id, 'skill_mastered', {
                            skill_id: selectedSkillId,
                            topic,
                        });
                        setStatus('complete');
