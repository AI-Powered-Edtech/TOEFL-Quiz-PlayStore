...
export const IELTSParagraphLevel: React.FC<{ onNavigate: (view: AppView) => void }> = ({ onNavigate }) => {
    ...
    const handleConfirmSelection = () => {
        ...
        if (currentStepIndex < exercise.ielts_data.steps.length - 1) {
            ...
            // Fire-and-forget analytics — paragraph builder completed
            writingGymService.logAnalyticsEvent(user?.id, 'paragraph_builder_completed', {
                band_score: newScore,
                task_prompt: exercise?.ielts_data?.task_prompt?.substring(0, 100),
            });
            setStatus('summary');
