...
export const writingGymService = {
    ...
    /**
     * Evaluate paragraph cohesion using AI
     */
    async evaluateParagraphCohesion(
        topicSentence: string,
        supportingDetails: string,
        concludingSentence: string,
        topic: string
    ): Promise<{ cohesionScore: number; logicalFlow: string; topicUnity: string; suggestions: string[] }> {
        try {
            const { callGroq, cleanJson } = await import('./groq/client');
            const { parseJsonSafely } = await import('./groq/utils/jsonParser');
            const { PARAGRAPH_COHESION_PROMPT } = await import('./groq/prompts/paragraphCohesionPrompts');
            const content = await callGroq(
                [
                    { role: 'system', content: PARAGRAPH_COHESION_PROMPT },
                    {
                        role: 'user',
                        content: `Topic: ${topic}\n\nTopic Sentence: ${topicSentence}\nSupporting Details: ${supportingDetails}\nConcluding Sentence: ${concludingSentence}`
                    }
                ],
                0.3,
                { jsonMode: true }
            );
            const raw = parseJsonSafely(cleanJson(content));
            return {
                cohesionScore: typeof raw.cohesionScore === 'number' ? raw.cohesionScore : 70,
                logicalFlow: raw.logicalFlow ?? 'The paragraph flows reasonably well.',
                topicUnity: raw.topicUnity ?? 'The ideas are related to the topic.',
                suggestions: Array.isArray(raw.suggestions) ? raw.suggestions : [],
            };
        } catch {
            return { cohesionScore: 0, logicalFlow: 'Unable to evaluate.', topicUnity: '', suggestions: [] };
        }
    },
};
