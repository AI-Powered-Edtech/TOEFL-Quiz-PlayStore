...
import { getUnifiedQuestionsBySkill, getAllQuestions, createQuestion, updateQuestion, deleteQuestion } from '../services/questionBankService';
...
export const BankView: React.FC<BankViewProps> = ({ onNavigate, onStartQuizWithQuestions }) => {
    ...
    const loadAllQuestions = async (page: number = 1, section: string = 'all') => {
        setIsLoading(true);
        try {
            const result = await getAllQuestions(page, questionsPerPage, section);
    ...
    const handleSearch = async () => {
        ...
        setHasSearched(true);
        try {
            const results = await getUnifiedQuestionsBySkill(searchSkillId, 50);
    ...
    useEffect(() => {
        const channel = supabase
            ...
            .on('postgres_changes',
                ...
                (payload) => {
                    console.log('[Realtime] Question bank changed:', payload);
                    if (viewMode === 'all') {
