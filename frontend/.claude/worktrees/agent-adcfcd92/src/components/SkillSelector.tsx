...
export const SkillSelector: React.FC<SkillSelectorProps> = ({ onSelectSkill, isLoading, hideHeader, initialSection, userId }) => {
  ...
  // Fetch per-skill stats from quiz_results, filtered by active section
  useEffect(() => {
    const fetchSkillStats = async () => {
      try {
        ...
        let query = supabase
          .from('quiz_results')
          .select('skill_id, correct_count, total_questions')
        ...
        if (error || !data) {
          console.warn('[SkillSelector] Failed to fetch skill stats:', error);
          return;
        }
        ...
        data.forEach((row: any) => {
          ...
          if (!statsMap[row.skill_id]) {
            statsMap[row.skill_id] = { accuracy: 0, quizCount: 0, totalCorrect: 0, totalQuestions: 0 };
          }
          statsMap[row.skill_id].quizCount += 1;
          statsMap[row.skill_id].totalCorrect += (row.correct_count || 0);
          statsMap[row.skill_id].totalQuestions += (row.total_questions || 0);
        });
