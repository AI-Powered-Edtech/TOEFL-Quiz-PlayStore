...
export interface CanonicalQuestionV1 {
  ...
  metadata: {
    ...
    hints?: string[];
    // Legacy mapping helpers (optional)
...
export interface ComplexityLevelProgress {
  id?: string;
  user_id?: string;
  skill_id: string;
...
export interface ComplexitySkillProgress {
  passedLevels: number[];      // indices of passed levels, e.g. [0, 1, 2]
  masteryPercentage: number;   // 0-100
  bestScores: Record<number, number>; // level_index → best score
}
