...
import { WritingExercise, WritingGymLevel, WritingGymProgress, IELTSWritingTask, ComplexityLadderLevel, ComplexitySkillProgress } from '../types';
...
/** Shape of items stored in the offline complexity progress queue. */
type ComplexityQueueItem = {
    userId: string;
    skillId: string;
    levelIndex: number;
    passed: boolean;
...
export const writingGymService = {
    ...
    async saveComplexityLevelProgress(
        userId: string,
        skillId: string,
        levelIndex: number,
        passed: boolean,
        ...
        const QUEUE_KEY = 'complexity_ladder_progress_queue';
        try {
            await supabase.from('complexity_ladder_progress').upsert({
                user_id: userId,
                skill_id: skillId,
                ...
                updated_at: new Date().toISOString(),
            }, { onConflict: 'user_id,skill_id,level_index' });
        } catch {
            const existing: ComplexityQueueItem[] = JSON.parse(localStorage.getItem(QUEUE_KEY) || '[]');
            existing.push({ userId, skillId, levelIndex, passed, score, userInput, topic, timestamp: Date.now() });
            localStorage.setItem(QUEUE_KEY, JSON.stringify(existing));
        }
    },

    async getComplexitySkillProgress(
        userId: string,
        skillId: string
    ): Promise<ComplexitySkillProgress> {
        const empty: ComplexitySkillProgress = { passedLevels: [], masteryPercentage: 0, bestScores: {} };
        try {
            const { data, error } = await supabase
                .from('complexity_ladder_progress')
                .select('level_index, passed, score')
                .eq('user_id', userId)
                .eq('skill_id', skillId);
            if (error || !data) return empty;
            type Row = { level_index: number; passed: boolean; score: number | null };
            const rows = data as Row[];
            const passedLevels = rows.filter(r => r.passed).map(r => r.level_index);
            const bestScores: Record<number, number> = {};
            rows.forEach(r => { if (r.score != null) bestScores[r.level_index] = r.score; });
            return {
                passedLevels,
                masteryPercentage: Math.round((passedLevels.length / 5) * 100),
                bestScores,
            };
        } catch {
            return empty;
    ...
    async flushComplexityProgressQueue(): Promise<void> {
        const QUEUE_KEY = 'complexity_ladder_progress_queue';
        const queued: ComplexityQueueItem[] = JSON.parse(localStorage.getItem(QUEUE_KEY) || '[]');
        if (queued.length === 0) return;
        const remaining: ComplexityQueueItem[] = [];
        for (const item of queued) {
            try {
                await this.saveComplexityLevelProgress(item.userId, item.skillId, item.levelIndex, item.passed, item.score, item.userInput, item.topic);
            } catch {
                remaining.push(item);
            }
        }
