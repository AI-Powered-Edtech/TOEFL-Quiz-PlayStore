...
import {
    ...
    AlignLeft, Undo2, Redo2, Copy, BarChart3, AlertTriangle
} from 'lucide-react';
import { Button } from '../../Button';
import { IELTSWritingTask, IELTSAssessment } from '../../../types';
import { TaskType, SimActiveTab } from './types';

type GrammarError = NonNullable<IELTSAssessment['grammar_errors']>[number];
            ...
            <div className="flex-1 max-w-4xl mx-auto w-full flex flex-col gap-4 overflow-hidden relative mt-2 md:mt-4">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[28px] shadow-sm flex-1 overflow-hidden flex flex-col border border-slate-100 dark:border-slate-800 mx-1 md:mx-0">
                    ...
                    <div className={`${activeTab === 'answer' ? 'flex' : 'hidden'} flex-1 flex-col h-full bg-white dark:bg-slate-900 relative`}>
                        {/* Revision Banner */}
                        {isRevising && (
                            <div className="flex items-center gap-2 px-4 py-2.5 bg-amber-50 border-b border-amber-200 text-amber-800 text-sm font-medium shrink-0">
                                <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0" />
                                Revising your previous submission — edit and resubmit
                            </div>
                        )}
                        ...
                        {/* Grammar Errors Panel (revision mode only) */}
                        {isRevising && grammarErrors && grammarErrors.length > 0 && (
                            <div className="border-t border-amber-100 bg-amber-50/60 dark:bg-slate-800/60 shrink-0">
                                <button
                                    onClick={() => setErrorsExpanded(prev => !prev)}
                                    className="w-full flex items-center justify-between px-4 py-2.5 text-sm font-bold text-amber-800 dark:text-amber-300"
                                    aria-expanded={errorsExpanded}
                                >
                                    <span className="flex items-center gap-2">
                                        <AlertTriangle className="w-4 h-4 text-amber-500" />
                                        Feedback Areas ({grammarErrors.length})
                                    </span>
                                        ...
                                        >
                                            <div className="px-4 pb-3 flex flex-wrap gap-2">
                                                {grammarErrors.map((err, i) => (
                                                    <span
                                                        key={i}
                                                        title={`Correction: ${err.correction} — ${err.explanation}`}
                                                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium cursor-help ${severityChipClass(err.severity)}`}
                                                    >
                                                        {err.fragment}
                                                    </span>
                                                ))}
