...
const ScoreComparisonBadge: React.FC<{ originalBandScore: number; revisedBandScore: number }> = ({ originalBandScore, revisedBandScore }) => {
    const diff = revisedBandScore - originalBandScore;
    ...
    return (
        <div className={`text-xs font-bold px-3 py-1 rounded-full mb-2 flex items-center gap-1 ${colorClass}`}>
            {improved ? '↑' : same ? '—' : '↓'}
            {' '}Original: {originalBandScore} → Revised: {revisedBandScore}
        </div>
    );
};
        ...
        <div className="h-full flex flex-col bg-[#F8FAFC] dark:bg-slate-950 overflow-hidden relative">
            <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm sticky top-0 z-20">
                ...
                <div className="flex flex-col items-center py-6">
                    ...
                    {/* Score comparison row (revision mode) */}
                    {isRevising && originalBandScore != null && (
                        <ScoreComparisonBadge
                            originalBandScore={originalBandScore}
                            revisedBandScore={assessment.band_score}
                        />
                    )}
