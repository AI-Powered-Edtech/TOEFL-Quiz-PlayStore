...
export const IELTSWritingSim: React.FC<{ onNavigate: (view: AppView) => void; onBack: () => void; }> = ({ onNavigate, onBack }) => {
    ...
    const [originalBandScore, setOriginalBandScore] = useState<number | null>(null);
    const [originalAssessment, setOriginalAssessment] = useState<IELTSAssessment | null>(null);
    ...
    const handleRevise = () => {
        setIsRevising(true);
        if (originalBandScore === null && assessment) {
            setOriginalBandScore(assessment.band_score);
            setOriginalAssessment(assessment);
        }
        setAssessment(null);
        setActiveTab('answer');
        setFeedbackTab('score');
        ...
        <div className={`h-[100dvh] flex flex-col bg-[#EEF2FC] dark:bg-slate-950 overflow-hidden ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
            ...
            {task && (
                <SimExamView
                    ...
                    isRevising={isRevising}
                    grammarErrors={originalAssessment?.grammar_errors}
                />
            )}
