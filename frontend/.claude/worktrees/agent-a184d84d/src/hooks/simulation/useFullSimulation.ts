        ...
        try {
            ...
            if (user) {
                const [reading, listening, structure, written] = await Promise.all([
                    getAnsweredQuestionIds(user.id, 'reading'),
                    getAnsweredQuestionIds(user.id, 'listening'),
                    getAnsweredQuestionIds(user.id, 'structure'),
                    getAnsweredQuestionIds(user.id, 'written'),
                ]);
