...
export const SimFeedbackView: React.FC<SimFeedbackViewProps> = ({
    ...
    originalBandScore, isRevision
}) => {
    const origScore = isRevision && originalBandScore != null ? originalBandScore : null;
    const scoreDelta = origScore !== null ? assessment.band_score - origScore : null;
    return (
        <div className="h-full flex flex-col bg-[#F8FAFC] dark:bg-slate-950 overflow-hidden relative">
            <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm sticky top-0 z-20">
                ...
                <div className="flex flex-col items-center py-6">
                    ...
                    {/* Score Comparison — shown only for revisions */}
                    {scoreDelta !== null && origScore !== null ? (
                        <div className={`mx-4 mb-3 px-4 py-3 rounded-2xl border flex items-center justify-center gap-3 text-sm font-bold ${
                            scoreDelta > 0
                            ...
                            <span className="text-slate-500 dark:text-slate-400">
                                Original: <span className="text-slate-800 dark:text-white">{origScore.toFixed(1)}</span>
                            </span>
                            <span className="text-slate-400">→</span>
                            <span className="text-slate-500 dark:text-slate-400">
                                Revised: <span className="text-slate-800 dark:text-white">{assessment.band_score.toFixed(1)}</span>
                            </span>
                            <span className={`ml-1 ${
                                scoreDelta > 0 ? 'text-green-600 dark:text-green-400' :
                                scoreDelta < 0 ? 'text-amber-600 dark:text-amber-400' :
                                'text-slate-500 dark:text-slate-400'
                            }`}>
                                {scoreDelta > 0 ? `↑ +${scoreDelta.toFixed(1)}` : scoreDelta < 0 ? `↓ ${scoreDelta.toFixed(1)}` : '→ 0.0'}
                            </span>
                        </div>
                    ) : null}
