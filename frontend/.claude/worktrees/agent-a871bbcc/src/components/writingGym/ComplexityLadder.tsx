...
export const ComplexityLadder: React.FC<Props> = ({ onNavigate }) => {
    ...
    const completedSessionsRef = useRef<Record<string, unknown>[]>([]);
    ...
    const loadProgress = async () => {
        ...
        try {
            ...
            completedSessionsRef.current = sessions as Record<string, unknown>[];
    ...
    const loadBestScores = useCallback(async (skillId: string) => {
        const cacheKey = `complexity_ladder_progress_cache_${userId}_${skillId}`;
        try {
            // Try the dedicated service method first (added in parallel unit)
            const service = writingGymService as Record<string, unknown>;
            if (typeof service['getComplexitySkillProgress'] === 'function') {
                const progress = await (service['getComplexitySkillProgress'] as (
                    uid: string, sid: string
                ) => Promise<{ bestScores?: Record<string, number> }>)(userId, skillId);
        ...
        try {
            ...
            if (cached) {
                const parsed = JSON.parse(cached) as { bestScores?: Record<string, number> };
        ...
        // Secondary fallback: derive from already-fetched sessions (no extra network call)
        const skillSessions = completedSessionsRef.current.filter(s => s['skill_id'] === skillId);
        if (skillSessions.length > 0) {
            // Use stars_earned as a proxy for score (* 33 to get /100 equivalent)
            const bestStars = Math.max(...skillSessions.map(s => Number(s['stars_earned']) || 0));
            if (bestStars > 0) {
                setBestScores({ [skillId]: Math.min(100, bestStars * 33) });
            }
        }
    }, [userId]);
            ...
            <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-6 gap-6 max-w-md mx-auto w-full">
                    ...
                    <div className="text-center space-y-2">
                        ...
                        {selectedSkillId && bestScores[selectedSkillId] !== undefined ? (
                            <div className="inline-flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-semibold px-3 py-1 rounded-full mt-1">
                                <Trophy className="w-3 h-3 text-amber-500" />
                                Best: {bestScores[selectedSkillId]}/100
                            </div>
                        ) : null}
                    </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between gap-2 flex-wrap">
                                    ...
                                    {bestScores[String(currentIdx)] !== undefined ? (
                                        <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[11px] font-semibold px-2.5 py-0.5 rounded-full shrink-0">
                                            <Trophy className="w-3 h-3 text-amber-500" />
                                            Best: {bestScores[String(currentIdx)]}/100
                                        </span>
                                    ) : null}
                                </div>
