...
export async function purchaseSubscription(
    ...
    try {
        // Get current user for appAccountToken
...
async function verifyAndActivate(
    ...
    try {
        console.log('[Purchase] Verifying purchase on server...');
        ...
        if (error) {
            ...
            return {
                success: false,
                error: 'Verifikasi pembelian gagal. Hubungi support jika saldo sudah terpotong.',
            };
        }

        if (data?.success) {
            // Clear tier cache so getUserTier() picks up the new tier
            clearTierCache();
