...
export const BlogListingView: React.FC<BlogListingViewProps> = ({ onNavigate }) => {
    ...
    useEffect(() => {
        const fetchProgress = async () => {
            if (!user) return;
            try {
                // Fetch completed unique skills per section
                const { data, error } = await supabase
                    .from('quiz_results')
                    .select('section, skill_id')
                    .eq('user_id', user.id);

                if (error) throw error;

                // Group by section and count unique skill_ids
                const completedSkills = data.reduce((acc: any, row: any) => {
                    if (!row.skill_id) return acc;
                    const sec = row.section.toUpperCase();
                    // map written to WRITTEN, structure to STRUCTURE (in DB both might be 'structure' or 'written' but historically 'written' is separate in skills.ts)
                    // Let's just do a simple mapping for now mostly looking at the design
                    const mappedSec = sec === 'WRITTEN' ? 'WRITTEN' : (sec === 'STRUCTURE' ? 'STRUCTURE' : (sec === 'LISTENING' ? 'LISTENING' : 'READING'));

                    if (!acc[mappedSec]) acc[mappedSec] = new Set();
                    acc[mappedSec].add(row.skill_id);
                    return acc;
                }, {});
